...
export default function ScenarioSimulator() {
  ...
  return (
    <section id="scenario" className="mx-auto max-w-4xl px-6 py-24">
      <Reveal>
        <h2 className="text-xs uppercase tracking-[0.3em] text-muted">Scenario Simulator</h2>
      </Reveal>
      <Reveal delay={0.05}>
        <p className="mt-2 max-w-xl text-2xl text-foreground">Shock a portfolio driver and see the expected-loss shift.</p>
      </Reveal>
        ...
        <form onSubmit={handleSubmit} className="mt-8 flex flex-wrap items-end gap-4">
          <div>
            ...
            <select
              ...
              className="mt-2 rounded border border-border bg-background-raised px-3 py-2 text-sm text-foreground focus:border-accent focus:outline-none"
            >
              {FEATURES.map((f) => (
          ...
          <div>
            ...
            <input
              ...
              className="mt-2 w-40 rounded border border-border bg-background-raised px-3 py-2 text-sm text-foreground focus:border-accent focus:outline-none"
            />
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="rounded bg-accent px-6 py-2 text-xs uppercase tracking-wide text-accent-foreground transition hover:opacity-90 disabled:opacity-40"
          >
        ...
        <Reveal delay={0.05} className="mt-12">
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-3">
            ...
            <div className="rounded-lg border border-border bg-background-raised p-6">
              ...
              <p className={`mt-2 font-mono text-2xl ${deltaPositive ? "text-danger" : "text-accent"}`}>
                {deltaPositive ? "+" : ""}
